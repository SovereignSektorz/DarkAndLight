...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		const definitions: Record<string, unknown> = {
			...
			Version: (process.env.DARK_MATTER_VERSION || (product as typeof product & { darkmatterVersion?: string }).darkmatterVersion || pkg.version),
			RawVersion: (process.env.DARK_MATTER_VERSION || (product as typeof product & { darkmatterVersion?: string }).darkmatterVersion || pkg.version).replace(/-\w+$/, ''),
			Commit: commit,
