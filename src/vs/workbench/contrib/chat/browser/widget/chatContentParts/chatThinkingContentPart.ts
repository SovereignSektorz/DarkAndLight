...
${this.hookCount > 0 ? `EXAMPLES WITH BLOCKED CONTENT (from hooks):
	...
	protected override setTitle(title: string, omitPrefix?: boolean): void {
		...
		// Ensure any file links generated by the LLM are rendered as file widgets by emptying their link text.
		// Matches: [some text](file:///path) and replaces with [](file:///path)
		const normalizedTitle = title.replace(/\[[^\]]*\]\((file:\/\/[^\)]+)\)/g, '[]($1)');

		const result = this.chatContentMarkdownRenderer.render(new MarkdownString(normalizedTitle));
